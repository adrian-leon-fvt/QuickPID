#ifndef _QUICK_PID_H_
#define _QUICK_PID_H_
...
public:
  ...
  QuickPID(float *Input, float *Output, float *Setpoint,
           ...
           pMode _pmode = pMode::pOnError,
           dMode _dmode = dMode::dOnMeas,
           iAwMode _iawmode = iAwMode::iAwCondition,
           Action _action = Action::direct);
  ...
  void SetControllerDirection(Action _Action);
  ...
  void SetAntiWindupMode(iAwMode _iAwMode);
  ...
  virtual uint32_t current_timestamp_in_us() const = 0;
  virtual uint32_t elapsed_time_in_us() const = 0;
...
private:
  ...
  uint32_t sampleTimeUs = 100'000U;
  uint32_t lastTime = 0U;
  float outputSum, outMin, outMax, error, lastError, lastInput;

  template <typename T>
  static T constrain(const T value, const T min, const T max);
