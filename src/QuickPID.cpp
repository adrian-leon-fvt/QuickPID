/**********************************************************************************
   QuickPID Library for Arduino - Version 1.0
   by adrian-leon-fvt https://github.com/adrian-leon-fvt/QuickPID
   Based on Dlloydev/QuickPID. Licensed under the MIT License.
 **********************************************************************************/
...
QuickPID::QuickPID(float *Input, float *Output, float *Setpoint, float Kp, float Ki, float Kd,
                   pMode _pmode, dMode _dmode, iAwMode _iawmode, Action _action)
{
  ...
  QuickPID::SetControllerDirection(_action);
  QuickPID::SetTunings(Kp, Ki, Kd, _pmode, _dmode, _iawmode);
}
...
QuickPID::QuickPID(float *Input, float *Output, float *Setpoint,
                   float Kp, float Ki, float Kd, Action _action)
...
bool QuickPID::Compute()
{
  if (mode == Control::manual)
    return false;
  uint32_t now = current_timestamp_in_us();
  uint32_t timeChange = elapsed_time_in_us();
...
void QuickPID::SetTunings(float Kp, float Ki, float Kd,
                          pMode _pMode, dMode _dMode, iAwMode _iAwMode)
{
...
/* SetSampleTime(.)***********************************************************
  Sets the period, in current_timestampeconds, at which the calculation is performed.
******************************************************************************/
...
******************************************************************************/
void QuickPID::SetControllerDirection(Action _Action)
{
  action = _Action;
}
...
******************************************************************************/
void QuickPID::SetProportionalMode(pMode _pMode)
{
  pmode = _pMode;
}
...
******************************************************************************/
void QuickPID::SetDerivativeMode(dMode _dMode)
{
  dmode = _dMode;
}
...
******************************************************************************/
void QuickPID::SetAntiWindupMode(iAwMode _iAwMode)
{
  iawmode = _iAwMode;
}
...
uint32_t QuickPID::GetLastTime() const
{
  return lastTime;
}

void QuickPID::InitLastTime()
{
    lastTime = current_timestamp_in_us();
}

/* Constrain function *********************************************************
  A local implementation of the constrain function
******************************************************************************/
template <typename T>
T QuickPID::constrain(const T value, const T min, const T max)
{
  T _value(value);
  T _min(min);
  T _max(max);

  // Ensure the minimum is always smaller than the maximum value
  if (_min > _max)
  {
    T temp(min);
